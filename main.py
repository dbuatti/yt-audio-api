...
def handle_audio_request():
    ...
    ydl_opts = {
        ...
        'extractor_args': {
            'youtube': {
                'player_client': ['mweb', 'ios', 'android'], # Focused on mobile clients
                'po_token': [f'web+${po_token}'] if po_token else [],
                'visitor_data': visitor_data if visitor_data else ""
            }
        },
        'http_headers': {
            # MATCHING YOUR EXTRACTED SESSION EXACTLY
...
def download_audio():
    ...
    if not token or not access_manager.has_access(token): return jsonify(error="Invalid token"), 401
    try:
    ...
    except: return jsonify(error="File error"), 404
